...
{
    // Wait for IRC to get it first - disabled with moneyenom
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Moneyenom " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
// testnet dns seed begins with 't', all else are moneyenom dns seeds.
static const char *strDNSSeed[][2] = {
    {"seed", "seed.moneyenom.net"},
    {"altcointech", "dnsseed.ppc.altcointech.net"},
    {"tnseed", "tnseed.moneyenom.net"},
};
...
void ThreadDNSAddressSeed2(void* parg)
{
    ...
    if (true /*!fTestNet*/)  // moneyenom enables dns seeding with testnet too
    {
...
bool BindListenPort(string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, sizeof(sockaddr)) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to port %d on this computer.  Moneyenom is probably already running."), ntohs(sockaddr.sin_port));
        else
...
void StartNode(void* parg)
{
    ...
    // moneyenom: mint proof-of-stake blocks in the background
