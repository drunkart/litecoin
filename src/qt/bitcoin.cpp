...
static void handleRunawayException(std::exception *e)
{
    ...
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occured. Moneyenom can no longer continue safely and will quit.") + QString("\n\n") + QString::fromStdString(strMiscWarning));
    exit(1);
}
...
// TODO: implement qtipcserver.cpp for Mac and Windows
    ...
    {
        if (strlen(argv[i]) >= 7 && strncasecmp(argv[i], "moneyenom:", 7) == 0)
        {
    ...
    app.setOrganizationName("Moneyenom");
    app.setOrganizationDomain("moneyenom.org");
    if(GetBoolArg("-testnet")) // Separate UI settings for testnet
        app.setApplicationName("Moneyenom-Qt-testnet");
    else
        app.setApplicationName("Moneyenom-Qt");
